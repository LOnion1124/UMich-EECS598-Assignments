...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, score_cache = Linear.forward(h, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        loss += (self.params['W1'] ** 2).sum() * self.reg
        loss += (self.params['W2'] ** 2).sum() * self.reg

        dh, dW2, db2 = Linear.backward(dscore, score_cache)
        dW2 += 2 * self.params['W2'] * self.reg
        dx, dW1, db1 = Linear_ReLU.backward(dh, h_cache)
        dW1 += 2 * self.params['W1'] * self.reg
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers - 1):
            self.params['W' + str(i + 1)] = weight_scale * torch.randn(hidden_dims[i - 1], hidden_dims[i], dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        h_cur, h_cache_cur = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])

        if self.use_dropout:
            h_cur, dropout_cache_cur = Dropout.forward(h_cur, self.dropout_param)
            dropout_cache.append(dropout_cache_cur)
        
        h.append(h_cur)
        h_cache.append(h_cache_cur)

        for i in range(1, self.num_layers - 1):
            h_cur, h_cache_cur = Linear_ReLU.forward(h[i], self.params['W' + str(i+1)], self.params['b' + str(i+1)])
            if self.use_dropout:
                h_cur, dropout_cache_cur = Dropout.forward(h_cur, self.dropout_param)
                dropout_cache.append(dropout_cache_cur)
        ...
        scores, score_cache = Linear.forward(h[-1], self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        dh = [0.0] * self.num_layers
        dW = [0.0] * (self.num_layers + 1)
        db = [0.0] * (self.num_layers + 1)

        dh[self.num_layers - 1], dW[self.num_layers], db[self.num_layers] = Linear.backward(dscore, score_cache)
        dW[self.num_layers] += 2 * self.params['W' + str(self.num_layers)] * self.reg

        for i in range(self.num_layers - 1, 1, -1):
            if self.use_dropout:
                dh[i] = Dropout.backward(dh[i], dropout_cache[i])

            dh[i - 1], dW[i], db[i] = Linear_ReLU.backward(dh[i], h_cache[i])
            dW[i] += 2 * self.params['W' + str(i)] * self.reg
        
        if self.use_dropout:
            dh[1] = Dropout.backward(dh[1], dropout_cache[1])

        dx, dW[1], db[1] = Linear_ReLU.backward(dh[1], h_cache[1])
        dW[1] += 2 * self.params['W1'] * self.reg
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data=data_dict, device=device, optim_config={'learning_rate': 0.1})
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
...
def get_three_layer_network_params():
    ...
    ###############################################################
    weight_scale = 0.58   # Experiment with this!
...
def get_five_layer_network_params():
    ...
    weight_scale = 0.45   # Experiment with this!
    # Replace "pass" statement with your code
    pass
    ################################################################
    #                       END OF YOUR CODE                       #
