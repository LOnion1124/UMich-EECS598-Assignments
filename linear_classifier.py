...
def svm_loss_vectorized(
    ...
    correct_scores = scores[torch.arange(X.shape[0]), y].view(-1, 1) # shape (N, 1)
    margins = scores - correct_scores + 1 # shape (N, C)
    margins[torch.arange(X.shape[0]), y] = 0 # Ignore correct scores
    ...
    #############################################################################
    # Replace "pass" statement with your code
    mask = (margins > 0).double() # shape (N, C) binary value
    count = mask.sum(dim = 1) # Count pos margins, shape (N, )
    mask[torch.arange(X.shape[0]), y] = -count
...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    _, y_pred = scores.max(dim=1) # shape(N,)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [8e-3, 9e-3, 1e-2, 1.1e-2, 1.2e-2]
    regularization_strengths = [2e-2]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    train_acc = torch.sum(cls.predict(data_dict['X_train']) == data_dict['y_train']) / data_dict['X_train'].shape[0]
    val_acc = torch.sum(cls.predict(data_dict['X_val']) == data_dict['y_val']) / data_dict['X_val'].shape[0]
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(N):
        ...
        scores -= torch.max(scores) # Numerical stability
        scores_exp = torch.exp(scores)
        probs = scores_exp / torch.sum(scores_exp)
        loss += -torch.log(probs[y[i]]) # Cross-Entropy

        # Gradient
        d_scores = torch.zeros_like(scores) # shape (C,)
        for j in range(C):
            if j == y[i]:
                d_scores[j] = probs[j] - 1
            else:
                d_scores[j] = probs[j]
        d_w = X[i].view(-1, 1).mm(d_scores.view(1, -1)) #shape(D, C)
        dW += d_w
    
    loss /= N
    dW /= N
    dW += reg * 2 * W * W # Regularization
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_loss_vectorized(
    ...
    # Loss
    scores = X @ W # shape (N, C)
    max_scores = scores.max(dim=1, keepdim=True).values # shape (N, 1)
    scores -= max_scores # Numerical stability
    scores_exp = torch.exp(scores)
    sum_scores = scores_exp.sum(dim=1, keepdim=True) # shape (N, 1)
    probs = scores_exp / sum_scores
    correct_probs = probs[torch.arange(N), y] # shape (N,)
    loss += -torch.log(correct_probs).sum()

    # Gradient
    y_onehot = torch.zeros_like(scores) # shape (N, C)
    y_onehot[torch.arange(N), y] = 1
    d_scores = probs - y_onehot
    dW = X.T @ d_scores # shape (D, C)
...
def softmax_get_search_params():
    ...
    - regularization_strengths: regularization strengths candidates
                                e.g. [1e0, 1e1, ...]
    """
    learning_rates = [9e-3, 1e-2, 1.1e-2]
    regularization_strengths = [0.08, 0.09]
