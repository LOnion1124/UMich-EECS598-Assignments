            ...
            for c in range(C):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        mask = (window == max_val)
                        dx[n, c, h_start:h_start+pool_height, w_start:w_start+pool_width] += mask * (dout[n, c, i, j] / mask.sum())
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        flatten_size = num_filters * H * W // 4 # Size of the flatten output of conv - relu - 2*2 max pool
        self.params['W2'] = weight_scale * torch.randn(flatten_size, hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        conv_out_flat = conv_out.view(conv_out.shape[0], -1)
        fc_out, fc_cache = Linear_ReLU.forward(conv_out_flat, W2, b2)
        scores, score_cache = Linear.forward(fc_out, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dfc_out, dw3, db3 = Linear.backward(dscores, score_cache)
        dconv_out_flat, dw2, db2 = Linear_ReLU.backward(dfc_out, fc_cache)
        dconv_out = dconv_out_flat.view(conv_out.shape)
        dx, dw1, db1 = Conv_ReLU_Pool.backward(dconv_out, conv_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        if weight_scale == 'kaiming':
            self.params['W1'] = kaiming_initializer(C, num_filters[0], K=3, relu=True, device=device, dtype=dtype)
        else:
            self.params['W1'] = weight_scale * torch.randn(num_filters[0], C, 3, 3, dtype=dtype, device=device)
        ...
        for i in range(1, self.num_layers - 1):
            ...
            b_name = 'b' + str(i + 1)
            if weight_scale == 'kaiming':
                self.params[W_name] = kaiming_initializer(num_filters[i - 1], num_filters[i], K=3, relu=True, device=device, dtype=dtype)
            else:
                self.params[W_name] = weight_scale * torch.randn(num_filters[i], num_filters[i - 1], 3, 3, dtype=dtype, device=device)
            self.params[b_name] = torch.zeros(num_filters[i], dtype=dtype, device=device)
        
        num_pools = len(max_pools)
        H_out = H // (2 ** num_pools)
        W_out = W // (2 ** num_pools)
        flat_size = num_filters[-1] * H_out * W_out

        if weight_scale == 'kaiming':
            self.params['W' + str(self.num_layers)] = kaiming_initializer(flat_size, num_classes, relu=False, device=device, dtype=dtype)
        else:
            self.params['W' + str(self.num_layers)] = weight_scale * torch.randn(flat_size, num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        #########################################################
        # Replace "pass" statement with your code
        conv_out = [None] * (self.num_layers - 1)
        conv_cache = [None] * (self.num_layers - 1)

        if self.batchnorm:
            gamma = self.params['gamma1']
            beta = self.params['beta1']
            if 0 in self.max_pools:
                conv_out[0], conv_cache[0] = Conv_BatchNorm_ReLU_Pool.forward(
                    X, self.params['W1'], self.params['b1'], gamma, beta, 
                    conv_param=conv_param, bn_param=self.bn_params[0], 
                    pool_param=pool_param)
            else:
                conv_out[0], conv_cache[0] = Conv_BatchNorm_ReLU.forward(
                    X, self.params['W1'], self.params['b1'], gamma, beta, 
                    conv_param=conv_param, bn_param=self.bn_params[0])
        else:
            if 0 in self.max_pools:
                conv_out[0], conv_cache[0] = Conv_ReLU_Pool.forward(
                    X, self.params['W1'], self.params['b1'], 
                    conv_param=conv_param, pool_param=pool_param)
            else:
                conv_out[0], conv_cache[0] = Conv_ReLU.forward(
                    X, self.params['W1'], self.params['b1'], 
        ...
        for i in range(1, self.num_layers - 1):
            ...
            if self.batchnorm:
                ...
                if i in self.max_pools:
                    conv_out[i], conv_cache[i] = Conv_BatchNorm_ReLU_Pool.forward(
                        conv_out[i - 1], self.params[W_name], self.params[b_name], gamma, beta,
                        conv_param=conv_param, bn_param=self.bn_params[i], pool_param=pool_param)
                else:
                        conv_out[i], conv_cache[i] = Conv_BatchNorm_ReLU.forward(
                            conv_out[i - 1], self.params[W_name], self.params[b_name], gamma, beta,
                            conv_param=conv_param, bn_param=self.bn_params[i])
            else:
                if i in self.max_pools:
                    conv_out[i], conv_cache[i] = Conv_ReLU_Pool.forward(conv_out[i - 1], self.params[W_name], self.params[b_name], conv_param=conv_param, pool_param=pool_param)
                else:
                    conv_out[i], conv_cache[i] = Conv_ReLU.forward(conv_out[i - 1], self.params[W_name], self.params[b_name], conv_param=conv_param)
        
        conv_out_flat = conv_out[-1].reshape(conv_out[-1].shape[0], -1)
        scores, scores_cache = Linear.forward(conv_out_flat, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        ...
        dconv_out = [None] * (self.num_layers - 1)
        dconv_out_flat, grads['W' + str(self.num_layers)], grads['b' + str(self.num_layers)] = Linear.backward(dscores, scores_cache)
        dconv_out[-1] = dconv_out_flat.view(conv_out[-1].shape)
        ...
        for i in range(self.num_layers - 2, 0, -1):
            ...
            if self.batchnorm:
                ...
                if i in self.max_pools:
                    dconv_out[i - 1], grads[W_name], grads[b_name], grads[gamma_name], grads[beta_name] = Conv_BatchNorm_ReLU_Pool.backward(dconv_out[i], conv_cache[i])
                else:
                    dconv_out[i - 1], grads[W_name], grads[b_name], grads[gamma_name], grads[beta_name] = Conv_BatchNorm_ReLU.backward(dconv_out[i], conv_cache[i])
            else:
                if i in self.max_pools:
                    dconv_out[i - 1], grads[W_name], grads[b_name] = Conv_ReLU_Pool.backward(dconv_out[i], conv_cache[i])
                else:
                    dconv_out[i - 1], grads[W_name], grads[b_name] = Conv_ReLU.backward(dconv_out[i], conv_cache[i])
        
        if self.batchnorm:
            if 0 in self.max_pools:
                dx, grads['W1'], grads['b1'], grads['gamma1'], grads['beta1'] = Conv_BatchNorm_ReLU_Pool.backward(dconv_out[0], conv_cache[0])
            else:
                dx, grads['W1'], grads['b1'], grads['gamma1'], grads['beta1'] = Conv_BatchNorm_ReLU.backward(dconv_out[0], conv_cache[0])
        else:
            if 0 in self.max_pools:
                dx, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dconv_out[0], conv_cache[0])
            else:
                dx, grads['W1'], grads['b1'] = Conv_ReLU.backward(dconv_out[0], conv_cache[0])
...
def find_overfit_parameters():
    weight_scale = 0.072   # Experiment with this!
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    # Replace "pass" statement with your code
    model = DeepConvNet(num_filters=[32] * 6, max_pools=[1, 3, 5], weight_scale='kaiming', dtype=dtype, device=device)
    solver = Solver(model, data_dict, batch_size=128, update_rule=adam, optim_config={'learning_rate': 1e-3}, device=device)
    #########################################################
    #                  END OF YOUR CODE                     #
    #########################################################
    return solver
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        dxhat = dout * gamma
        dvar = (dxhat * (x - mean) * -0.5 * (var + eps)**(-1.5)).sum(dim=0)
        dmean = (dxhat * -1 / std).sum(dim=0)
    ...
    def backward_alt(dout, cache):
        ...
        dxhat = dout * gamma
        dmean = dxhat.sum(dim=0) / std
        dvar = (dxhat * (x - mean)).sum(dim=0) * (-0.5) / (var + eps) / std
        
        dx = dxhat / std - dmean / N - dvar * 2 * (x - mean) / N
        #################################################################
        #                        END OF YOUR CODE                       #
        #################################################################
