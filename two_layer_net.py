...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = (X @ W1) + b1.view(1, -1) # shape (N, H)
    hidden[hidden <= 0] = 0 # ReLU
    scores = (hidden @ W2) + b2.view(1, -1) # shape (N, C)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    scores -= scores.max(dim=1, keepdim=True).values # shape (N, C)
    scores_exp = torch.exp(scores)
    probs = scores_exp / scores_exp.sum(dim=1, keepdim=True) # Softmax, shape (N, C)
    probs_correct = probs[torch.arange(N), y] # shape (N,)
    loss = -torch.log(probs_correct).sum() / N
    loss += reg * ((W1 * W1).sum() + (W2 * W2).sum()) # Regularaization
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    d_scores = probs.clone() # shape (N, C)
    d_scores[torch.arange(N), y] -= 1
    d_scores /= N
    # Second layer
    d_W2 = h1.T @ d_scores + 2 * reg * W2 # shape (H, C)
    d_b2 = d_scores.sum(dim=0) # shape (C,)
    d_h1 = d_scores @ W2.T
    # First layer
    d_z = d_h1.clone() # shape (N, H) for ReLU grad
    d_z[h1 <= 0] = 0;
    d_W1 = X.T @ d_z  + 2 * reg * W1# shape (D, H)
    d_b1 = d_z.sum(dim=0) # shape (H,)
...
def nn_get_search_params():
    ...
    hidden_sizes = [64]
    regularization_strengths = [3e-5, 5e-5, 7e-5]
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    lrs, hss, rss, lrds = nn_get_search_params()
    for lr in lrs:
        for hs in hss:
            for rs in rss:
                for lrd in lrds:
                    print("train with lr: {}, hs: {}, rs: {}, lrd: {}".format(lr, hs, rs, lrd))
                    ...
                    cur_acc = stats["val_acc_history"][-1]
                    print("get val acc: {}".format(cur_acc))
