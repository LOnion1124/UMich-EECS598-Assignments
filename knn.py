...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      point1 = x_train[i].view(-1)
      for j in range(num_test):
        point2 = x_test[j].view(-1)
        diff = point1 - point2
        dis = (diff * diff).sum()
        dists[i, j] = dis
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
      point = x_train[i].view(-1)
      diff = x_test.view(num_test, -1) - point
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_squared = (x_train_flat ** 2).sum(dim=1, keepdim=True)
    x_test_squared = (x_test_flat ** 2).sum(dim=1, keepdim=True)
    cross_term = x_train_flat @ x_test_flat.T

    dists = x_train_squared + x_test_squared.T - 2 * cross_term
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    _, indices = dists.topk(k=k, dim=0, largest=False)
    y_nearest = y_train.view(num_train, 1).broadcast_to(num_train, num_test)[indices, torch.arange(num_test)]
    y_pred, _ = y_nearest.mode(dim=0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        ...
        x_train_cv = torch.cat(x_train_folds[:i] + x_train_folds[i+1:])
        y_train_cv = torch.cat(y_train_folds[:i] + y_train_folds[i+1:])
