...
def gram_matrix(features, normalize=True):
    ...
    f = features.reshape(N, C, H * W)          # (N, C, H*W)
    gram = torch.bmm(f, f.transpose(1, 2))     # (N, C, C)
    if normalize:
      gram = gram / (C * H * W)
    ############################################################################
    #                               END OF YOUR CODE                           #
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
       ...
       style_loss += style_weights[i] * ((gram_cur - style_targets[i]) ** 2).sum()
...
def tv_loss(img, tv_weight):
    ...
    diff_y = img[:, 1:, :] - img[:, :-1, :] # (3, H-1, W)
    diff_x = img[:, :, 1:] - img[:, :, :-1] # (3, H, W-1)
    loss = tv_weight * ((diff_x ** 2).sum() + (diff_y ** 2).sum())
    return loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
