...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss_real = nn.functional.binary_cross_entropy_with_logits(logits_real, target_real, reduction='mean')
    target_fake = torch.zeros_like(logits_fake)
    loss_fake = nn.functional.binary_cross_entropy_with_logits(logits_fake, target_fake, reduction='mean')
...
def generator_loss(logits_fake):
    ...
    loss = nn.functional.binary_cross_entropy_with_logits(logits_fake, target, reduction='mean')
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    loss = (0.5 * (scores_real - 1) ** 2 + 0.5 * scores_fake ** 2).sum()
    loss /= scores_real.shape[0]
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
