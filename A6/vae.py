...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.hidden_dim = 256  # A common choice for hidden dimensions
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        N = x.shape[0]
        x = x.view(N, -1)
        x_with_class = torch.cat([x, c], dim=-1) # (N, H*W + C)
        enc_out = self.encoder(x_with_class)
        mu = self.mu_layer(enc_out)
        logvar = self.logvar_layer(enc_out)
        z = reparametrize(mu, logvar) # (N, Z)
        z_with_class = torch.cat([z, c], dim=-1) # (N, Z + C)
        x_hat = self.decoder(z_with_class)
        ###########################################################################
        #                                      END OF YOUR CODE                   #
        ###########################################################################
        return x_hat, mu, logvar
...
def reparametrize(mu, logvar):
    ...
    # Replace "pass" statement with your code
    N, Z = mu.shape
    epsilon = torch.randn(N, Z, device=mu.device)
...
def loss_function(x_hat, x, mu, logvar):
    ...
    reconstruction_loss = F.binary_cross_entropy(x_hat_flat, x_flat, reduction='sum') / x.size(0)
