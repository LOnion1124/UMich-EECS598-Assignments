...
def prepocess_input_sequence(
    ...
    for token in tokens:
        if token in spc_tokens:
            ...
            # is number
            out += [token_dict[d] for d in token]
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    similarities = (query @ key.t()).divide(math.sqrt(M)) # (K, K)
    attn_weights = F.softmax(similarities, dim=-1) # (K, K)
    out = attn_weights @ value # (K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    similarities = torch.matmul(query, key.transpose(dim0=1, dim1=2)).divide(math.sqrt(M)) # (N, K, K)
    attn_weights = F.softmax(similarities, dim=-1) # (N, K, K)
    out = torch.matmul(attn_weights, value) # (N, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(similarities, dim=-1) # (N, K, K)
    y = torch.matmul(weights_softmax, value) # (N, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        # Replace "pass" statement with your code
        c_q, c_k, c_v = (
            math.sqrt(6 / (dim_in + dim_q)),
            math.sqrt(6 / (dim_in + dim_q)),
            math.sqrt(6 / (dim_in + dim_v))
        )
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        for i in range(num_heads):
            single_heads.append(SelfAttention(dim_in, dim_q=dim_out, dim_v=dim_out))
        self.multi_head = nn.ModuleList(single_heads)
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        num_heads = len(self.multi_head)
        N, K, M = query.shape
        ys = []
        for single_head in self.multi_head:
            out = single_head(query, key, value, mask)
            ys.append(out)
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        lin_out = self.linear1(x)
        lin_out = self.relu(lin_out)
        y = self.linear2(lin_out)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.multihead_attn = MultiHeadAttention(num_heads, dim_in=emb_dim, dim_out=(emb_dim // num_heads))
        self.norm1 = LayerNormalization(emb_dim)
        self.norm2 = LayerNormalization(emb_dim)
        self.feedforward = FeedForwardBlock(inp_dim=emb_dim, hidden_dim_feedforward=feedforward_dim)
    ...
    def forward(self, x):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        attn_out = self.multihead_attn(x, x, x)
        attn_out = self.dropout(attn_out)
        out2 = self.norm1(attn_out + x)
        ff_out = self.feedforward(out2)
        ff_out = self.dropout(ff_out)
        y = self.norm2(ff_out + out2)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # Replace "pass" statement with your code
        self.attention_self = MultiHeadAttention(num_heads, dim_in=emb_dim, dim_out=(emb_dim // num_heads))
        self.attention_cross = MultiHeadAttention(num_heads, dim_in=emb_dim, dim_out=(emb_dim // num_heads))
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = (torch.arange(0, K) / K).unsqueeze(dim=-1).expand(-1, M).unsqueeze(dim=0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    pos = torch.arange(0, K).unsqueeze(1)  # (K, 1)
    dim_indices = torch.arange(0, M, 2)  # even indices
    
    denominator = torch.pow(10000.0, dim_indices // M)
    
    y = torch.zeros(K, M)
    
    y[:, 0::2] = torch.sin(pos / denominator)
    if M % 2 == 0:
        y[:, 1::2] = torch.cos(pos / denominator)
    else:
        y[:, 1::2] = torch.cos(pos / denominator[:-1])
    
    # Add batch dimension
    y = y.unsqueeze(0)  # (1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        mask = get_subsequent_mask(seq=ans_b[:, :-1])
