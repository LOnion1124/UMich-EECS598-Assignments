...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = F.tanh(prev_h @ Wh + x @ Wx + b)
...
def rnn_step_backward(dnext_h, cache):
    ...
    dtanh = (1 - next_h ** 2) * dnext_h  # derivative of tanh
    dx = dtanh @ Wx.t()
    dprev_h = dtanh @ Wh.t()
    dWx = x.t() @ dtanh
    dWh = prev_h.t() @ dtanh
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        x_step = x[:, t, :].view(N, D)
        h_t, cache_t = rnn_step_forward(x_step, h_t, Wx, Wh, b)
        h[:, t, :] = h_t
...
def rnn_backward(dh, cache):
    ...
    dprev_h_t = torch.zeros(N, H, dtype=dh.dtype, device=dh.device)

    for t in reversed(range(T)):
        dnext_h = dh[:, t, :] + dprev_h_t
        dx_t, dprev_h_t, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, cache[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def temporal_softmax_loss(x: torch.Tensor, y, ignore_index=None):
    """
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        self.word_embedding = WordEmbedding(len(word_to_idx), wordvec_dim)
        if cell_type == "rnn":
            ...
            self.attn_proj = nn.Conv2d(self.image_encoder.out_channels, hidden_dim, kernel_size=1)
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        # (1) Encode images and project to initial hidden state
        img_features = self.image_encoder(images)  # (N, C, H_img, W_img)
        if self.cell_type == "attn":
            A = self.attn_proj(img_features)  # (N, H, 4, 4)
            h0 = A.mean(dim=(2, 3))  # (N, H)
        else:
            pooled = img_features.mean(dim=(2, 3))  # (N, C)
            h0 = self.feature_projection(pooled)    # (N, H)

        # (2) Embed input captions
        wordvecs = self.word_embedding(captions_in)  # (N, T, W)

        # (3) RNN/LSTM/AttentionLSTM forward
        if self.cell_type == "rnn":
            hiddens = self.rnn(wordvecs, h0)  # (N, T, H)
        elif self.cell_type == "lstm":
            hiddens = self.lstm(wordvecs, h0)  # (N, T, H)
        elif self.cell_type == "attn":
            hiddens = self.attn_lstm(wordvecs, A)  # (N, T, H)

        # (4) Output projection to vocab scores
        scores = self.output_projection(hiddens)  # (N, T, V)
    ...
    def sample(self, images: torch.Tensor, max_length=15):
        """
        ...
        if self.cell_type == "attn":
            ...
            pooled = img_features.mean(dim=(2, 3))  # (N, C)
            h = self.feature_projection(pooled)      # (N, H)
            if self.cell_type == "lstm":
                c = torch.zeros_like(h)

        # (2) Initialize captions with <START> token
        captions = torch.full((images.shape[0], max_length), self._null, dtype=torch.long, device=images.device)
        prev_word = torch.full((images.shape[0],), self._start, dtype=torch.long, device=images.device)

        for t in range(max_length):
            # (3) Embed previous word
            word_embed = self.word_embedding(prev_word.view(-1, 1))  # (N, 1, W)
            word_embed = word_embed.squeeze(1)  # (N, W)

            # (4) RNN/LSTM/AttentionLSTM step
            if self.cell_type == "rnn":
                h = self.rnn.step_forward(word_embed, h)
            elif self.cell_type == "lstm":
                h, c = self.lstm.step_forward(word_embed, h, c)
            elif self.cell_type == "attn":
                attn, attn_weights = dot_product_attention(h, A)
                h, c = self.attn_lstm.step_forward(word_embed, h, c, attn)
                attn_weights_all[:, t] = attn_weights

            # (5) Output projection to vocab scores
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(T):
            x_step = x[:, t, :].view(N, D)
            h_t, c_t = self.step_forward(x_step, h_t, c_t)
            hn[:, t, :] = h_t
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def dot_product_attention(prev_h, A):
    ...
    prev_h_exp = prev_h.unsqueeze(2)  # (N, H, 1)
    scores = (A_flat * prev_h_exp).sum(dim=1) / math.sqrt(H)  # (N, 16)
    attn_weights = F.softmax(scores, dim=1)  # (N, 16)
    attn = (A_flat * attn_weights.unsqueeze(1)).sum(dim=2)  # (N, H)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(T):
            ...
            h_t, c_t = self.step_forward(x_step, h_t, c_t, attn)
            hn[:, t, :] = h_t
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
