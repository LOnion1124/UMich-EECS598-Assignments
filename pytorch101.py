...
def create_tensor_of_pi(M: int, N: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    def make_list(length, item):
      res = []
      for i in range(length):
        res.append(item)
      return res
    
    tmp = make_list(M, make_list(N, 3.14))
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mean = x.sum(dim = 0) / M
    tmp = x - mean
    std = ((tmp * tmp).sum(dim = 0) / (M - 1)).sqrt()
    y = (x - mean) / std
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
